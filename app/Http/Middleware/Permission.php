...
class Permission
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
      ...
      if(in_array($curren_route_name, $links)){
            ...
            if($request->Ajax()){
                return response()->json([
                    'status' => 404,
                    'data' => null,
                    'route' => $curren_route_name,
                    'sms' => 'អ្នកមិនមានសិទ្ធចូលប្រើប្រាស់',
                ]);
            }
            return redirect()->back()->with('error', 'អ្នកមិនមានសិទ្ធចូលប្រើប្រាស់!');
        }
        return $next($request);
